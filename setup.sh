...
function do_raspios {
  ...
  # Remove unnecessary packages. Use || true for packages that may not exist.
  run sudo apt-get -y remove \
    'gcc-4*' 'gcc-5*' 'gcc-6*' 'gcc-7*' 'gcc-8*' 'gcc-9*' 'gcc-10*' \
    python3-rpi.gpio triggerhappy 2>/dev/null || true
...
function do_ssh {
  ...
  if [ -f "$SSH_KEY" ]; then
    cat "$SSH_KEY" >>"$HOME_DIR/.ssh/authorized_keys"
  fi
  if [ "$USERNAME" != "root" ]; then
...
function do_wifi {
  ...
  if (which nmcli > /dev/null); then
    # RaspiOS Bookworm+ and C.H.I.P. use NetworkManager.
    # Check if the connection already exists.
    if ! nmcli -t -f NAME connection show 2>/dev/null | grep -q "^${WIFI_SSID}$"; then
      run sudo nmcli device wifi connect "$WIFI_SSID" password "$WIFI_PASS"
    else
      echo "  Wifi connection ${WIFI_SSID} already exists, skipping"
    fi
  elif (which connmanctl > /dev/null); then
...
function do_nmconnection {
  echo "- do_nmconnection: Copy NetworkManager connection profiles from /boot"
  if [ $BANNER_ONLY -eq 1 ]; then return 0; fi
  ...
  for f in /boot/*.nmconnection; do
    ...
    run sudo cp "$f" "$DST/$NAME"
    run sudo chmod 600 "$DST/$NAME"
    run sudo chown root:root "$DST/$NAME"
  done
...
function do_all {
  ...
  # Copy any .nmconnection files from /boot, so wifi is up before setup runs.
  do_nmconnection
